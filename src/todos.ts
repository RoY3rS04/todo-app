import Todo, { TodoActions, isTodoArray } from "./todo";
...
if (addBtn instanceof HTMLFormElement) {
    addBtn.addEventListener('submit', (e) => {
        e.preventDefault();
        const todoInfo = new FormData(e.target as HTMLFormElement);

        const todo: Todo = new Todo(
            todoInfo.get('title') as string,
            todoInfo.get('description') as string,
            false
        )
    ...
    deleteBtns.forEach((btn) => {
        if (btn instanceof HTMLFormElement) {
            btn.addEventListener('submit', (e) => {
                ...
                TodoActions.deleteTodo(data.get('id') as string);
                loadDOM();
            })
        }
    })
        ...
        checkTodos.forEach((check) => {
            if (check instanceof HTMLInputElement) {
                check.addEventListener('change', (e) => {
                    TodoActions.updateTodo((e.target as HTMLInputElement).value)
                })
            }
        })
    }
...
function trashIcon(color: string = 'currentColor') {
    return `
